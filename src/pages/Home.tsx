...
const getTomorrowActionStorageKey = (storeId: string, date: string) =>
  `openSignal.tomorrowActions.${storeId}.${date}`;

const readCompletedTomorrowActions = (storeId: string, date: string) => {
  try {
    return JSON.parse(
      localStorage.getItem(getTomorrowActionStorageKey(storeId, date)) || "[]",
    ) as string[];
  } catch {
    return [];
  }
...
const writeCompletedTomorrowActions = (
  storeId: string,
  date: string,
  actionIds: string[],
) => {
  localStorage.setItem(
    getTomorrowActionStorageKey(storeId, date),
    JSON.stringify(actionIds),
  );
};

const getDemoLogStorageKey = (storeId: string) =>
  `openSignal.dailyBusinessLogs.${storeId}`;

const readDemoLogs = (storeId: string) => {
  try {
    return JSON.parse(
      localStorage.getItem(getDemoLogStorageKey(storeId)) || "[]",
    ) as DailyBusinessLog[];
  } catch {
    return [];
  }
...
export default function Home() {
  ...
  const [weeklyWeatherData, setWeeklyWeatherData] = useState<
    { date: string; dow: string; condition: string }[]
  >([]);
  ...
  const [completedTomorrowActions, setCompletedTomorrowActions] = useState<
    string[]
  >([]);

  useEffect(() => {
    if (!user) return;
    setCompletedTomorrowActions(
      readCompletedTomorrowActions(user.id, tomorrowDate),
    );
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (!fError && forecastData && forecastData.length > 0) {
          ...
          setEventFactorsDetail(
            tomorrow.event_factors_detail || "상세 정보가 제공되지 않았습니다.",
          );
        }
        ...
        const { data: logData, error: logError } = await supabase
          ...
          .select(
            "id, log_date, total_sales_amount, customer_count, weather, special_event_memo, business_memo, source",
          )
          .eq("store_id", storeId)
          .gte("log_date", start)
          .lte("log_date", end)
        ...
        const proofItems =
          proofError && isMissingTableError(proofError)
            ? readStoredProofItems(storeId, weekRange.start)
            : ((proofData ||
                proofTaskTitles.map((taskTitle) => ({
        ...
        const { data: expenseData, error: expenseError } = await supabase
          .from("expenses")
          .select(
            "id, expense_date, category, amount, vendor, memo, receipt_status, recurring_template_id",
          )
          .eq("store_id", storeId)
        ...
        const weeklyExpenses =
          expenseError && isMissingTableError(expenseError)
            ? readStoredExpenses(storeId).filter(
                (expense) =>
                  expense.expense_date >= weekRange.start &&
                  expense.expense_date <= weekRange.end,
              )
            : ((expenseData || []) as ExpenseRecord[]).filter(
                (expense) =>
                  expense.expense_date >= weekRange.start &&
                  expense.expense_date <= weekRange.end,
              );
        ...
        const completedProofCount = proofItems.filter(
          (item) => item.is_completed,
        ).length;
        setProofSummary({
          completionRate:
            proofItems.length > 0
              ? Math.round((completedProofCount / proofItems.length) * 100)
              : 0,
          missingExpenseCount: weeklyExpenses.filter(
            (expense) =>
              expense.receipt_status === "missing" ||
  ...
  const getWeatherIcon = (cond: string) => {
    ...
    if (cond.includes("구름") || cond.includes("흐림"))
      return <Cloud size={16} className="text-slate-500" />;
    return <Sun size={16} className="text-orange-500" />;
  };

  const monthlySalesTotal = businessLogs.reduce(
    (sum, log) => sum + (log.total_sales_amount || 0),
    0,
  );
  const monthlyCustomerTotal = businessLogs.reduce(
    (sum, log) => sum + (log.customer_count || 0),
    0,
  );
  const monthlyExpenseTotal = expenseRecords.reduce(
    (sum, expense) => sum + (expense.amount || 0),
    0,
  );
  const averageTicket =
    monthlyCustomerTotal > 0
      ? Math.round(monthlySalesTotal / monthlyCustomerTotal)
      : null;
  ...
  const recentAverageSales =
    recentLogs.length > 0
      ? Math.round(
          recentLogs.reduce((sum, log) => sum + log.total_sales_amount, 0) /
            recentLogs.length,
        )
      : 0;
  ...
  const tomorrowActions = (() => {
    ...
    const hasStockoutSignal =
      memoText.includes("품절") ||
      memoText.includes("재고") ||
      memoText.includes("소진");
    const hasStaffSignal =
      memoText.includes("알바") ||
      memoText.includes("직원") ||
      memoText.includes("결근") ||
      memoText.includes("단체");
    const hasEventSignal =
      memoText.includes("학교") ||
      memoText.includes("축제") ||
      memoText.includes("행사") ||
      memoText.includes("시험");
    const latestSalesGap =
      recentAverageSales > 0
        ? Math.round(
            ((latestLog.total_sales_amount - recentAverageSales) /
              recentAverageSales) *
              100,
          )
        : 0;
    ...
    if (
      (tomorrowDayLabel === "금" || tomorrowDayLabel === "토") &&
      !hasRainSignal
    ) {
      addAction({
        id: "weekend-prep",
    ...
    if (actions.length < 2) {
      addAction({
        ...
        reason:
          latestLog.business_memo || latestLog.special_event_memo
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1">
            <img
              src={logoImg}
              alt="내:일 로고"
              className="w-7 h-7 object-contain translate-y-[2px]"
            />
            ...
            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-64 p-3 bg-slate-800 text-white text-xs rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
              ...
              <p className="text-slate-300">
                ...
                <span className="text-violet-300">
                  업종, 가게 위치(지역), 그리고 향후 7일간의 날씨 예보
                </span>{" "}
                데이터를 종합적으로 분석하여 예상 방문객 수를 논리적으로
                추정합니다.
              </p>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p
                      className={`text-sm font-bold leading-relaxed ${completedTomorrowActions.includes(action.id) ? "text-slate-400 line-through" : "text-slate-800"}`}
                    >
                      {action.title}
                    </p>
      ...
      <section className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
            ...
            <p className="text-lg font-bold text-slate-800">
              {formatWon(monthlySalesTotal - monthlyExpenseTotal)}
            </p>
            <p className="text-[11px] font-medium text-slate-400 mt-1">
              지출 미입력 기준
            </p>
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-2xl p-3">
              ...
              <p className="text-lg font-bold text-slate-800">
                {proofSummary.missingExpenseCount}건
              </p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-3">
              <p className="text-xs font-bold text-slate-500 mb-1">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar snap-x">
          {insights.length > 0 ? (
            insights.map((insight) => (
              <div
                key={insight.id}
                className="min-w-[280px] bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 flex flex-col snap-center"
              >
                <div className="h-32 bg-slate-200 relative">
                  ...
                  <img
                    ...
                    onError={(e) => {
                      e.currentTarget.src =
                        "https://images.unsplash.com/photo-1550505096-17b1287c8051?q=80&w=600&auto=format&fit=crop";
                    }}
                  />
            ...
            >
              <div className="p-6">
                  ...
                  {weeklyWeatherData.length > 0 ? (
                    weeklyWeatherData.map((w, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`flex items-center justify-between p-3 rounded-2xl ${isTomorrow ? "bg-violet-50 border border-violet-100" : "bg-slate-50"}`}
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-10 h-10 rounded-full flex items-center justify-center ${isTomorrow ? "bg-violet-100" : "bg-white shadow-sm"}`}
                            >
                            ...
                            <div>
                              <div className="font-bold text-slate-800 flex items-center gap-2">
                                {w.dow}요일{" "}
                                {isTomorrow && (
                                  <span className="text-[10px] bg-violet-600 text-white px-1.5 py-0.5 rounded-md font-bold">
                                    내일
                                  </span>
                                )}
                              </div>
                ...
                <p className="text-[10px] text-slate-400 mt-4 text-center">
                  * 미래의 날씨 및 예상 방문객은 AI를 통해 분석된 결과입니다.
                  (제공: OpenWeatherMap, OpenAI)
                </p>
              </div>
            </motion.div>
          </div>
